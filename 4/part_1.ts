...
fs.readFile("input.txt", "utf8", (err, data) => {
  ...
  pairs.forEach((pair) => {
    ...
    // First group is to check that sections 1 fully overlaps sections 2, the second group is the other way around
    if (
      (sections_1_start <= sections_2_start &&
        sections_1_end >= sections_2_end) ||
      (sections_2_start <= sections_1_start && sections_2_end >= sections_1_end)
    )
      count += 1;
  });

  console.log(count, "section pairs fully overlap");
});
