...
fs.readFile("input.txt", "utf8", (err, data) => {
  ...
  moves.forEach((move) => {
    ...
    // Grab amount of pieces from the end of from_col stack
    const pieces = stacks_obj[from_col].splice(
      stacks_obj[from_col].length - amount,
      amount
    );
    // Add them to the end of to_col stack
    stacks_obj[to_col] = stacks_obj[to_col].concat(pieces);
  });
